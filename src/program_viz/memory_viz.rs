...
pub struct MemoryVisualizer<'a> {
    program: &'a Program,
}

impl<'a> Widget for &MemoryVisualizer<'a> {
        ...
        Self: Sized,
    {
        ...
        self.render_address_area(address_area, buf);
        self.render_memory_area(memory_read_area, buf);
        self.render_ascii_area(ascii_area, buf);
    }
}

impl<'a> MemoryVisualizer<'a> {
    pub fn new(program: &'a Program) -> Self {
        Self { program }
    }

    fn render_address_area(&self, area: Rect, buf: &mut Buffer) {
    ...
    fn render_memory_area(&self, area: Rect, buf: &mut Buffer) {
        let mut memory_str = String::with_capacity(24 * 32);
        for row in 0u8..32 {
            for col in 0u8..8 {
                let b = self.program.memory_at(row * 8 + col);
    ...
    fn render_ascii_area(&self, area: Rect, buf: &mut Buffer) {
